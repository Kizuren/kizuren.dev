<template>
    <header class="p-4">
      <nav class="flex justify-between items-center">
        <div class="logo">
            <ULink as="button" :to="localePath('/')" class="cursor-default">{{ t.name }}</ULink>
        </div>
        <ul class="flex gap-6 md:gap-10">
          <li><ULink :to="localePath('/hire')">{{ t.nav.hire }}</ULink></li>
          <li><ULink :to="localePath('/contact')">{{ t.nav.contact }}</ULink></li>
